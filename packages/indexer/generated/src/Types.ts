// This file is to dynamically generate TS types
// which we can't get using GenType
// Use @genType.import to link the types back to ReScript code

import type { Logger, EffectCaller } from "envio";
import type * as Entities from "./db/Entities.gen.ts";

export type LoaderContext = {
  /**
   * Access the logger instance with event as a context. The logs will be displayed in the console and Envio Hosted Service.
   */
  readonly log: Logger;
  /**
   * Call the provided Effect with the given input.
   * Effects are the best for external calls with automatic deduplication, error handling and caching.
   * Define a new Effect using createEffect outside of the handler.
   */
  readonly effect: EffectCaller;
  /**
   * True when the handlers run in preload mode - in parallel for the whole batch.
   * Handlers run twice per batch of events, and the first time is the "preload" run
   * During preload entities aren't set, logs are ignored and exceptions are silently swallowed.
   * Preload mode is the best time to populate data to in-memory cache.
   * After preload the handler will run for the second time in sequential order of events.
   */
  readonly isPreload: boolean;
  /**
   * Per-chain state information accessible in event handlers and block handlers.
   * Each chain ID maps to an object containing chain-specific state:
   * - isReady: true when the chain has completed initial sync and is processing live events,
   *            false during historical synchronization
   */
  readonly chains: {
    [chainId: string]: {
      readonly isReady: boolean;
    };
  };
  readonly DisabledDelegation: {
    /**
     * Load the entity DisabledDelegation from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.DisabledDelegation_t | undefined>,
    /**
     * Load the entity DisabledDelegation from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.DisabledDelegation_t>,
    readonly getWhere: Entities.DisabledDelegation_indexedFieldOperations,
    /**
     * Returns the entity DisabledDelegation from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.DisabledDelegation_t) => Promise<Entities.DisabledDelegation_t>,
    /**
     * Set the entity DisabledDelegation in the storage.
     */
    readonly set: (entity: Entities.DisabledDelegation_t) => void,
    /**
     * Delete the entity DisabledDelegation from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
  readonly EnabledDelegation: {
    /**
     * Load the entity EnabledDelegation from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.EnabledDelegation_t | undefined>,
    /**
     * Load the entity EnabledDelegation from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.EnabledDelegation_t>,
    readonly getWhere: Entities.EnabledDelegation_indexedFieldOperations,
    /**
     * Returns the entity EnabledDelegation from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.EnabledDelegation_t) => Promise<Entities.EnabledDelegation_t>,
    /**
     * Set the entity EnabledDelegation in the storage.
     */
    readonly set: (entity: Entities.EnabledDelegation_t) => void,
    /**
     * Delete the entity EnabledDelegation from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
  readonly Redemption: {
    /**
     * Load the entity Redemption from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.Redemption_t | undefined>,
    /**
     * Load the entity Redemption from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.Redemption_t>,
    readonly getWhere: Entities.Redemption_indexedFieldOperations,
    /**
     * Returns the entity Redemption from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.Redemption_t) => Promise<Entities.Redemption_t>,
    /**
     * Set the entity Redemption in the storage.
     */
    readonly set: (entity: Entities.Redemption_t) => void,
    /**
     * Delete the entity Redemption from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
  readonly Stats: {
    /**
     * Load the entity Stats from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.Stats_t | undefined>,
    /**
     * Load the entity Stats from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.Stats_t>,
    readonly getWhere: Entities.Stats_indexedFieldOperations,
    /**
     * Returns the entity Stats from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.Stats_t) => Promise<Entities.Stats_t>,
    /**
     * Set the entity Stats in the storage.
     */
    readonly set: (entity: Entities.Stats_t) => void,
    /**
     * Delete the entity Stats from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
};

export type HandlerContext = {
  /**
   * Access the logger instance with event as a context. The logs will be displayed in the console and Envio Hosted Service.
   */
  readonly log: Logger;
  /**
   * Call the provided Effect with the given input.
   * Effects are the best for external calls with automatic deduplication, error handling and caching.
   * Define a new Effect using createEffect outside of the handler.
   */
  readonly effect: EffectCaller;
  /**
   * Per-chain state information accessible in event handlers and block handlers.
   * Each chain ID maps to an object containing chain-specific state:
   * - isReady: true when the chain has completed initial sync and is processing live events,
   *            false during historical synchronization
   */
  readonly chains: {
    [chainId: string]: {
      readonly isReady: boolean;
    };
  };
  readonly DisabledDelegation: {
    /**
     * Load the entity DisabledDelegation from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.DisabledDelegation_t | undefined>,
    /**
     * Load the entity DisabledDelegation from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.DisabledDelegation_t>,
    /**
     * Returns the entity DisabledDelegation from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.DisabledDelegation_t) => Promise<Entities.DisabledDelegation_t>,
    /**
     * Set the entity DisabledDelegation in the storage.
     */
    readonly set: (entity: Entities.DisabledDelegation_t) => void,
    /**
     * Delete the entity DisabledDelegation from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
  readonly EnabledDelegation: {
    /**
     * Load the entity EnabledDelegation from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.EnabledDelegation_t | undefined>,
    /**
     * Load the entity EnabledDelegation from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.EnabledDelegation_t>,
    /**
     * Returns the entity EnabledDelegation from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.EnabledDelegation_t) => Promise<Entities.EnabledDelegation_t>,
    /**
     * Set the entity EnabledDelegation in the storage.
     */
    readonly set: (entity: Entities.EnabledDelegation_t) => void,
    /**
     * Delete the entity EnabledDelegation from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
  readonly Redemption: {
    /**
     * Load the entity Redemption from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.Redemption_t | undefined>,
    /**
     * Load the entity Redemption from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.Redemption_t>,
    /**
     * Returns the entity Redemption from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.Redemption_t) => Promise<Entities.Redemption_t>,
    /**
     * Set the entity Redemption in the storage.
     */
    readonly set: (entity: Entities.Redemption_t) => void,
    /**
     * Delete the entity Redemption from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
  readonly Stats: {
    /**
     * Load the entity Stats from the storage by ID.
     * If the entity is not found, returns undefined.
     */
    readonly get: (id: string) => Promise<Entities.Stats_t | undefined>,
    /**
     * Load the entity Stats from the storage by ID.
     * If the entity is not found, throws an error.
     */
    readonly getOrThrow: (id: string, message?: string) => Promise<Entities.Stats_t>,
    /**
     * Returns the entity Stats from the storage by ID.
     * If the entity is not found, creates it using provided parameters and returns it.
     */
    readonly getOrCreate: (entity: Entities.Stats_t) => Promise<Entities.Stats_t>,
    /**
     * Set the entity Stats in the storage.
     */
    readonly set: (entity: Entities.Stats_t) => void,
    /**
     * Delete the entity Stats from the storage.
     *
     * The 'deleteUnsafe' method is experimental and unsafe. You should manually handle all entity references after deletion to maintain database consistency.
     */
    readonly deleteUnsafe: (id: string) => void,
  }
};
